...
export default function Relatorio() {
  ...
  const filtered = useMemo(
    () =>
      lancamentos
        .filter((item) => !categoriaId || String(item.categoriaId) === String(categoriaId))
      ...
      <PageTitle
        ...
        actions={
          <div className="grid gap-2 sm:grid-cols-[180px_120px_220px]">
            <select className="field" value={mes} onChange={(event) => setMes(event.target.value)}>
            ...
            <input className="field" type="number" value={ano} onChange={(event) => setAno(event.target.value)} />
            <select className="field" value={categoriaId} onChange={(event) => setCategoriaId(event.target.value)}>
        ...
        <table className="w-full min-w-[900px] border-collapse">
          ...
          <tbody>
            ...
            <tr className="bg-black/20">
              ...
              <td className="table-cell text-right font-semibold">{money(total)}</td>
            </tr>
          </tbody>
        </table>
      </div>
